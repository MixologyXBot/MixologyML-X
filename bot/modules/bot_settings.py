...
async def load_config():
    ...
    VERCEL_DOMAIN = environ.get('VERCEL_DOMAIN', '')
    if len(VERCEL_DOMAIN) == 0:
        VERCEL_DOMAIN = ''

    VERCEL_PROTECT_KEY = environ.get('VERCEL_PROTECT_KEY', '')
    if len(VERCEL_PROTECT_KEY) == 0:
        VERCEL_PROTECT_KEY = ''

    VERCEL_TOKEN_TTL = environ.get('VERCEL_TOKEN_TTL', '')
    VERCEL_TOKEN_TTL = int(VERCEL_TOKEN_TTL) if VERCEL_TOKEN_TTL.isdigit() else 0
...
<a href='{url}'>Read More ...</a>'''
    ...
    config_dict.update({'ANIME_TEMPLATE': DEF_ANI_TEMP,
                        ...
                        'VERCEL_PROTECT_KEY': VERCEL_PROTECT_KEY,
                        'VERCEL_TOKEN_TTL': VERCEL_TOKEN_TTL,
